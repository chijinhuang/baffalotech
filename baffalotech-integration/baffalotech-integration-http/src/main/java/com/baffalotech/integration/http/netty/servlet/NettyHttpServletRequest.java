...
import com.baffalotech.integration.http.netty.core.ApplicationPart;
import com.baffalotech.integration.http.netty.core.NettyHttpRequest;
import com.baffalotech.integration.http.netty.core.constants.HttpConstants;
import com.baffalotech.integration.http.netty.core.constants.HttpHeaderConstants;
...
import com.baffalotech.integration.http.netty.core.util.StringUtil;
...
import com.baffalotech.integration.http.netty.servlet.util.SnowflakeIdWorker;
import com.baffalotech.integration.http.netty.session.Session;
 ...
 *
 * @author acer01 2018/7/15/015
 */
@sun.misc.Contended
public class NettyHttpServletRequest implements javax.servlet.http.HttpServletRequest, Recyclable {

	private static final AbstractRecycler<NettyHttpServletRequest> RECYCLER=new AbstractRecycler<NettyHttpServletRequest>(){@Override protected NettyHttpServletRequest newInstance(){return new NettyHttpServletRequest();}};
	...
	private NettyServletAsyncContext asyncContext;
	...
	private boolean parsePartsFlag = false;
	...
	public static NettyHttpServletRequest newInstance(HttpServletObject httpServletObject,
			FullHttpRequest fullHttpRequest) {
		Objects.requireNonNull(httpServletObject);

		NettyHttpServletRequest instance = RECYCLER.getInstance();
	 ...
	 * 查询字符串和POST请求的数据被汇总到请求参数集合中。查询字符串数据在POST数据之前发送。例如， 如果请求由查询字符串a =hello
	 * 和POST数据a=goodbye&a=world 组成，得到的参数集合顺序将是 =(hello,goodbye,world)。
	 * 这些API不会暴露GET请求（HTTP 1.1所定义的）的路径参数。他们必须从getRequestURI方法或getPathInfo
	 * 方法返回的字符串值中解析。
	 *
	 * 以下是在POST表单数据填充到参数集前必须满足的条件： 1。该请求是一个HTTP或HTTPS请求。 2。HTTP方法是POST。
	...
	private void decodeParameter() {
		...
		if (HttpConstants.POST.equalsIgnoreCase(getMethod()) && getContentLength() > 0
	...
	/**
	 * 处理multipart-form的数据，即文件上传
	 */
	private void parseParts() {
		if (!parsePartsFlag) {
			if (HttpConstants.POST.equalsIgnoreCase(getMethod()) && HttpHeaderUtil.isMultiPart(getContentType())) {
				Exception partsParseException = null;
	...
	private void decodePaths() {
		...
		// 1.加上pathInfo
		// 直接计算pathInfo
		MappedResource<NettyServletRegistration> mappedResource = getServletContext()
				.getServletMappedResource(requestURI);
		if (mappedResource != null) {
			// 匹配上
			this.pathInfo = mappedResource.getPathSpec().getPathInfo(requestURI);
		}
	 ...
	 * servlet标准 :
	 *
	 * 返回指定请求头的值 作为long值，代表a 日期对象。使用这种方法 包含日期的标头，例如 返回日期为 从1970年1月1日开始的毫秒数。
	 * 头名不区分大小写。 ，如果请求没有页眉 指定名称，此方法返回-1。如果消息头 不能转换为日期，方法抛出。
	 ...
	 * servlet标准:
	 *
	 * 返回指定请求头的值 作为int。如果请求没有标题 指定的名称，此方法返回-1。如果 该方法不能将header转换为整数
	 * 抛出一个NumberFormatException 代码。 头名不区分大小写。
	 * 
	...
	@Override
	public NettyHttpServletSession getSession(boolean create) {
		NettyHttpServletSession httpSession = httpServletObject.getSession();
		...
		if (httpSession == null) {
			httpSession = new NettyHttpServletSession(servletContext);
		} else {
	...
	public String changeSessionId() {
		NettyHttpServletSession httpSession = getSession(true);
	...
	public boolean isRequestedSessionIdValid() {
		...
		return sessionIdSource == SessionTrackingMode.COOKIE || sessionIdSource == SessionTrackingMode.URL;
	}
	...
	public String getRequestedSessionId() {
		...
		String cookieSessionName = StringUtil.isNotEmpty(userSettingCookieName) ? userSettingCookieName
				: HttpConstants.JSESSION_ID_COOKIE;
	...
	@Override
	public NettyServletInputStream getInputStream() throws IOException {
		if (reader != null) {
	...
	@Override
	public NettyServletAsyncContext startAsync() throws IllegalStateException {
		return startAsync(this, httpServletObject.getHttpServletResponse());
	}

	@Override
	public NettyServletAsyncContext startAsync(ServletRequest servletRequest, ServletResponse servletResponse)
		...
		if (asyncContext == null) {
			asyncContext = new NettyServletAsyncContext(httpServletObject, servletContext,
					servletContext.getAsyncExecutorService(), servletRequest, servletResponse);
		}
	...
	@Override
	public NettyServletAsyncContext getAsyncContext() {
		return asyncContext;
	}
	...
	public Part getPart(String name) throws IOException, ServletException {
		if (this.getParts() != null) {
			for (Part part : this.getParts()) {
				if (StringUtils.equals(part.getName(), name)) {
					return part;
				}
			}
		}
