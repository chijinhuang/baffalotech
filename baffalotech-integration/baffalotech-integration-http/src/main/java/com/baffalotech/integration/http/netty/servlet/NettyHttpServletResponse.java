...
import com.baffalotech.integration.http.netty.core.util.*;
 ...
 */
@sun.misc.Contended
public class NettyHttpServletResponse implements javax.servlet.http.HttpServletResponse,Recyclable {

    private static final AbstractRecycler<NettyHttpServletResponse> RECYCLER = new AbstractRecycler<NettyHttpServletResponse>() {
        @Override
        protected NettyHttpServletResponse newInstance() {
            return new NettyHttpServletResponse();
        }
    };
    ...
    private final NettyServletOutputStreamWrapper outWrapper = new NettyServletOutputStreamWrapper(new CloseListener());
    private long contentLength = -1;
    private final AtomicInteger errorState = new AtomicInteger(0);


    protected NettyHttpServletResponse() {
    }

    public static NettyHttpServletResponse newInstance(HttpServletObject httpServletObject) {
        Objects.requireNonNull(httpServletObject);

        NettyHttpServletResponse instance = RECYCLER.getInstance();
    ...
    public void changeToChunkStream() {
        ...
        synchronized (outWrapper) {
            NettyServletOutputStream oldOut = outWrapper.unwrap();
            if(oldOut instanceof NettyServletOutputChunkedStream){
                return;
            }

            NettyServletOutputStream newOut = new NettyServletOutputChunkedStream();
    ...
    public NettyServletOutputStreamWrapper getOutputStream() throws IOException {
        if(outWrapper.unwrap() == null){
            synchronized (outWrapper) {
                if(outWrapper.unwrap() == null) {
                    outWrapper.wrap(NettyServletOutputStream.newInstance(httpServletObject));
                }
            }
        }
        return outWrapper;
    ...
    public boolean isCommitted() {
        ...
        NettyServletOutputStream out = outWrapper.unwrap();
    ...
    private class CloseListener implements ChannelFutureListener{
        ...
        public void operationComplete(ChannelFuture future) throws Exception {
            ...
            NettyHttpServletResponse.RECYCLER.recycleInstance(NettyHttpServletResponse.this);
        }
    }
}
